...
(Ribbon은 일정 주기마다 각각의 setServersList Ping을 날려서 상태를 확인한다.)

2. 왜 2번 서버를 죽었다고 판단하지 않는 걸까?  
1번 의문을 해결하면서 당황스러웠던 점은 Ribbon이 서버리스트를 갱신할 때에만 2번 서버를 죽었다고 인식하지 못한다는 점이었다.  
...
이상한 점은 Ping을 쏘기 전에 setServersList() 메소드를 실행해서 서버 상태를 전부 true로 바꾼다는 것이다.
